...
//	    loadDevName();
	...
	private void treReadDevcfg() {
		boolean msw = MultiScreenSwitch.getSwitchState(this);
		String name = SystemDevName.tryRead();
	    this.mDevNameTextView.setText(name);
	    setSWState(msw);
		Log.e("dsafds", "SW=" + msw + " Name=" + name);
	}
...
//	      AnyPlayHisense.this.mAPServerBinder.stopBonjour();
	...
	private void setSWState(boolean sw) {
	...
	private void EvtOnoff() {
	    ...
	    if (this.mONOFFButton.getText().toString().endsWith(str1)) {
		    ...
		    LocalInfo.isSwitchON = false;
		    this.mAPController.AirplayStop();
