...
import cn.airjoy.android.service.SystemDevName;
...
public class APService extends Service {
	...
	public void onCreate() {
	    ...
	    mLocalInfo.setName(SystemDevName.tryRead());
	    ...
	    localIntentFilter2.addAction(AnyPlayUtils.ACTION_HS_SW);
	    localIntentFilter2.addAction(AnyPlayUtils.ACTION_HS_NAME);
	...
	private BroadcastReceiver tv_event = new BroadcastReceiver() {
	    public void onReceive(Context paramContext, Intent paramIntent) {
	      ...
	      if (str.equals("com.hisense.TVSearchStart")) {
	        ...
	        APService.this.startAirJoy();
	      }else if (str.equals(AnyPlayUtils.ACTION_HS_SW)) {
	    	  Bundle bundle = paramIntent.getExtras();
	    	  boolean sw = bundle.getBoolean("on");
	    	  mSystemConfig.setSwitchON(sw);
	    	  if(sw) {
		        APService.this.startAirJoy();
	    	  }else {
		        APService.this.stopAirpaly();
			  }
	    	  Log.e("sdfdsafds", "sdfaaaaaaaaaaaaaa = " + str);
	      }else if (str.equals(AnyPlayUtils.ACTION_HS_NAME)) {
	    	  Log.e("sdfdsafds", "sdfaaaaaaaaaaaaaa = " + str);
	    	  String name = SystemDevName.tryRead();
		      APService.this.re_name(name);
	      }
	    }
	};
