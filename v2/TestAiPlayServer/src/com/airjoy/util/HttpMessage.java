...
public class HttpMessage {
    ...
    public LoadStatus loadBytes(byte[] bytes, int len) {
        ...
        String contentLength = this.getValueByName(HttpMessage.ContentLengthName);
    ...
    // convert HttpMessage to bytes stream
    public byte[] toBytes() {
        ...
        if (mType == Type.Request) {
            header = String.format("%s %s HTTP/%d.%d\r\n", mMethod, mUri, mVersionMajor,
                    mVersionMinor);
        }
        else {
            header = String.format("HTTP/%d.%d %d %s\r\n", mVersionMajor, mVersionMinor,
